...
/**
 * StudentPageLayout - Reusable layout for all student pages
 * Handles navigation, responsive layout, sidebar, top nav, and bottom nav
 */
export default function StudentPageLayout({ children, title = '' }) {
  const navigate = useNavigate();
  const [showMobileMenu, setShowMobileMenu] = useState(false);

  const navItems = [
    { label: 'Dashboard', icon: 'dashboard', route: '/student/dashboard' },
    { label: 'Marketplace', icon: 'shopping_bag', route: '/student/marketplace' },
    { label: 'Treasury', icon: 'account_balance_wallet', route: '/student/gifts' },
    { label: 'Academy', icon: 'school', route: '/study-session' },
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-surface-container border-b border-outline-variant/20">
        <div className="flex items-center justify-between px-6 py-3 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="hidden sm:block text-lg font-bold text-on-surface">EduCoin</h1>
          </div>
